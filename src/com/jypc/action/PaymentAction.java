...
import com.jypc.dao.PaymentDao;
...
public class PaymentAction extends ActionSupport {
	...
	private List<PaymentBean> paymentList;// 住户固定缴费信息
	...
	public void initData() {
		pager.setAllData(paymentDao.getDataNum());
		paymentList = paymentDao.getPaymentList(pager);// 获取固定费用收费信息列表（分页）
		costList = paymentDao.getCostList();
	}
	...
	public void existsPayId() throws IOException {
		...
		int result = paymentDao.existsPayId(id) ? 1 : 0;
		out.print(result);
		out.flush();// 刷新
		out.close();// 关闭
	}
	...
	public void exists() throws IOException {
		...
		int result = paymentDao.exists(tenementId, costId, selYears, selMonths) ? 1
				: 0;
		out.print(result);
		out.flush();// 刷新
		out.close();// 关闭
	...
	public void existsIdCard() throws IOException {
		...
		int result = paymentDao.existsIdCard(idCard) ? 1 : 0;
		out.print(result);
		out.flush();// 刷新
		out.close();// 关闭
	}
	...
	public String paymentAdd() {
		...
		result = paymentDao.paymentAdd(model);
		if (result > 0) {
			tips = "添加成功！";
		} else {
			tips = "添加失败！";
	...
	public String delInfo() {
		int result = paymentDao.delInfo(id);
		if (result > 0) {
			setTips("删除成功！");
		} else {
			setTips("删除失败！");
	...
	public String delPaymentLists() {
		...
		for (String item : paymentList) {
			result += paymentDao.delInfo(item);
		}
	...
	public void getPaymentInfo() throws IOException {
		...
		out.print(JSONObject.fromObject(paymentDao.getEditInfo(id)));
		out.flush();
		out.close();
	}
	...
	public void getTenementInfo() throws IOException {
		tenementList = paymentDao.getTenementInfo(idCard);
		HttpServletResponse response = ServletActionContext.getResponse();
		response.setContentType("text/html");
		response.setCharacterEncoding("UTF-8");
		PrintWriter out = response.getWriter();// 获取out
	...
	public String paymentEdit() {
		int result = 0;
		result = paymentDao.paymentUpdate(model);
		if (result > 0) {
			tips = "修改成功！";
		} else {
			tips = "修改失败！";
